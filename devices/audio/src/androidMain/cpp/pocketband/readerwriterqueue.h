...
// A lock-free wqueue for a single-consumer, single-producer architecture.
// The wqueue is also wait-free in the common path (except if more memory
...
// Using the wqueue exclusively from one thread is fine, though a bit silly.
...
{
	// Design: Based on a wqueue-of-queues. The low-level queues are just
	// circular buffers with front and tail indices indicating where the
	// next element to dequeue is and where the next element can be enqueued,
	// respectively. Each low-level wqueue is called a "block". Each block
	// wastes exactly one element's worth of space to keep the design simple
	// (if front == tail then the wqueue is empty, and can't be full).
	// The high-level wqueue is a circular linked list of blocks; again there
...
public:
	...
	// Constructs a wqueue that can hold at least `size` elements without further
	...
	// Note: The wqueue should not be accessed concurrently while it's
	...
	AE_NO_TSAN ~ReaderWriterQueue()
	{
		...
		// Destroy any remaining objects in wqueue and free memory
	...
	// Enqueues a copy of element if there is room in the wqueue.
	...
	// Enqueues a moved copy of element if there is room in the wqueue.
	// Returns true if the element was enqueued, false otherwise.
	// Does not allocate memory.
	...
	// Enqueues a copy of element on the wqueue.
	...
	// Enqueues a moved copy of element on the wqueue.
	// Allocates an additional block of memory if needed.
	// Only fails (returns false) if memory allocation fails.
	...
	// Attempts to dequeue an element; if the wqueue is empty,
	// returns false instead. If the wqueue has at least one element,
	...
	// Returns a pointer to the front element in the wqueue (the one that
	// would be removed next by a call to `try_dequeue` or `pop`). If the
	// wqueue appears empty at the time the method is called, nullptr is
	// returned instead.
	...
	// Removes the front element from the wqueue, if any, without returning it.
	// Returns true on success, or false if the wqueue appeared empty at the time
	// `pop` was called.
	bool pop() AE_NO_TSAN
	{
#ifndef NDEBUG
	...
	// Returns the approximate number of items currently in the wqueue.
	...
	// an allocation when this wqueue is empty.
	// Safe to call from both the producer and consumer threads.
	//
